...
import FotoBauAberto from "./2-fotoBauAberto";
import FotoBauFechado from "./1-fotoBauFechado";
...
export default function CheckListDevolucao({ setStep, id }: CheckListDevolucaoProps) {
  console.log(id)
  const [currentStep, setCurrentStep] = useState<Step>('fotoBauFechado')

  return (
    <Tabs defaultValue="fotoBauFechado" value={currentStep} onValueChange={setCurrentStep as (value: string) => void}>
      ...
      <TabsContent value="fotoBauFechado">
        <FotoBauFechado setCurrentStep={setCurrentStep as (step: string) => void} id={id}/>
      </TabsContent>
      ...
      <TabsContent value="temperatura">
        <Temperatura setCurrentStep={setCurrentStep as (step: string) => void} id={id}/>
      </TabsContent>
      <TabsContent className="p-4" value="anomalia">
        <Anomalia setStep={setStep} setCurrentStep={setCurrentStep as (step: string) => void} id={id}/>
      </TabsContent>
    </Tabs>
  )
}